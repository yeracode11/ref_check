 ...
 * @param {{ managerIds: string[], fridgeIdCandidates: string[], lng: number, lat: number, now: number, windowMs: number, maxDistanceM: number }} params
...
function findRecentDuplicateCheckin(candidates, params) {
  ...
  for (const c of candidates) {
    if (!fridgeIdMatchesCandidates(c.fridgeId, fridgeIdCandidates)) continue;
    if (!managerIdMatchesCandidates(c.managerId, managerIds)) continue;
